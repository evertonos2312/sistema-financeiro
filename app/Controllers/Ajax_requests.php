...
class Ajax_requests extends BaseController
{
	...
	{
		$cep = preg_replace('/\D/', '', $this->body['cep']);
        $md = md5($cep."CONTMATIC");
        $result = @file_get_contents("http://cep.contmatic.com.br/consultacep/json/{$cep}/{$md}");
		$decoded = json_decode($result, true);
		
		if($decoded['mensagem']){
			$this->AjaxLib->setError('0x001', $decoded['mensagem']);
		}else{
			$this->AjaxLib->setSuccess($decoded);
		}
	}
	
	public function checkExistCPF()
	{
		$this->AjaxLib->CheckRequired(['cpf']);
		$cpf = MaskCPF(preg_replace('/\D/', '', $this->body['cpf']));
		
		$mdl = new \App\Models\Participantes\Participantesmodel();
		$mdl->select = 'id';
		$mdl->where['tipo'] = 'nao_cliente';
		$mdl->where['cpf'] = $cpf;
		$exist = $mdl->search(1)[0];
		$exist = ($exist) ? true : false;
		
		$this->AjaxLib->setSuccess($exist);
	}
	...
	public function getCursosList()
	{
		...
		$ns_mdl = "\\App\\Models\\Admin\\Cursos\\Cursosmodel";
		$mdl = new $ns_mdl();
		
		$mdl->select = "id, codigo, nome";
		
		$mdl->where['BEGINORWHERE_nome'] = $this->body['search_param'];
		$mdl->where['ENDORWHERE_codigo'] = $this->body['search_param'];
		$results = $mdl->search(5);
		...
		if($results){
			...
			foreach($results as $key => $fields){
				$return[$key] = array(
					'value' => $fields['nome'],
					'label' => '['.$fields['codigo'].'] '.$fields['nome'],
					'id' => $fields['id'],
				);
			}
		}
	...
	public function getRegistro()
	{
		...
		//Protection against Ajax Post methods not authorized
		if(strpos($this->body['mdl'], 'Participantes') !== false
		&& $this->body['id'] != $this->session->get('auth_part')['id']){
			$this->AjaxLib->setError('1x001', 'Not authorized');
		}

		$this->body['mdl'] = "\\App\\Models\\".str_replace('/','\\',$this->body['mdl']);
		if(class_exists($this->body['mdl'])){
			$mdl = new $this->body['mdl']();

			if($this->body['fields']){
				$mdl->select = $this->body['fields'];
			}

			$mdl->f['id'] = $this->body['id'];
			$result = $mdl->get();

			if(!$result){
				$this->AjaxLib->setError('0x001', 'record not found');
			}else{
				$this->AjaxLib->setSuccess($result);
			}
		}
		$this->AjaxLib->setError('0x000', 'model not found');
	}

	public function getVagasTurma()
	{
		$this->AjaxLib->CheckRequired(['id']);
		$mdl = new \App\Models\Turmas\Turmasmodel();
		$return = $mdl->getDetalhesVagas($this->body['id']);
