	...
	*/
	function get_where()
	{
			...
			if(is_array($this->where)){
				foreach($this->where as $field => $value){
					...
					if(is_array($value)){
						...
						|| $this->fields_map[$field]['type'] == 'dropdown'
						|| strpos($field, '.deletado') !== false
						|| strpos($field, '.id') !== false){
							$cond_value = " = '{$value}'"; //FOR FIELDS RELATED, DROPDOWN, ID OR DELETED ITS EQUAL
						}else{
							$cond_value = " LIKE '%{$value}%'"; //DEFAULT ITS LIKE %%
						}
					}
		...
		if(!$this->force_deletado){
			...
			foreach($this->join as $table_name => $on){
				$new_table = explode(' as ', str_replace(['LEFTJOIN_', 'RIGHTJOIN_'], '', $table_name));
				$new_table = $new_table[count($new_table)-1];
				$return_string .= $c_where."{$new_table}.deletado = '0'";
				$c_where = ' AND ';
			}
		}
	...
	public function saveRecord($execute_logics = true)
	{
		...
		if(!empty($this->f['id']) && !$this->new_with_id){
			...
			$this->f['usuario_modificacao'] = $this->session->get('auth_user')['id'];
			$this->helper->where('id', $this->f['id']);
			...
			$this->f['usuario_criacao'] = $this->session->get('auth_user')['id'];
			$this->f['data_modificacao'] = date("Y-m-d H:i:s");
			$this->f['usuario_modificacao'] = $this->session->get('auth_user')['id'];
	...
	public function checkUploadFiles()
	{
				...
				if($value){
					if ($value->isValid() && !$value->hasMoved()){
						if($this->table != 'arquivos'){
							...
							$executed = $arquivos->helper->update(['arquivo' => $arquivos->f['id']]);
		...
		if(!empty($update)){
			$this->helper->where('id', $this->f['id']);
			$executed = $this->helper->update($update);
		}
	}
	...
	public function formatSingleRecordView($record, $ignore_raw){
		$record = (array) $record;
		foreach($this->fields_map as $field => $options){
			$value = $record[$field];
			...
			if($options['type'] == 'currency'){
				...
				switch($options['type']){
					...
					case 'dropdown':
						...
						break;
					case 'multidropdown':
						$value = $this->formaMultidropdown($options['parameter'], $value);
						break;
					case 'related':
						...
						$record[$field_nome] = ($related) ? $related['nome'] : '';
						break;
					case 'file':
	...
	public function formaMultidropdown($which, $value)
	{
