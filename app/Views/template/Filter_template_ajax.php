<form class="mr-auto" method="post" id="filtroForm{$id_filter}" action="{$action}">
	...
	{if $generic_filter}
		...
		{if !empty($filters)}
		<button class="btn btn-outline-success btn-rounded" type="button" data-toggle="modal" data-target="#filtroFormAdvancedModal"><i class="fas fa-search"></i> Busca Avançada {$icon_filter_advanced}</button>
		{/if}
	{/if}
