...
class LayoutLib
{
	...
	public function __construct($fields_map)
	{
		...
		$this->DropdownLib = new \App\Libraries\DropdownLib();
	...
	public function GetField($field, $attrs, $record)
	{
		...
		switch($attrs['type']){
			...
			case 'cep':
				...
				break;
			case 'dropdown':
				$selected = (!is_null($record['raw'][$field])) ? $record['raw'][$field] : ((isset($attrs['default'])) ? $attrs['default'] : '');
				...
				break;
			case 'multidropdown':
				$selected = (!is_null($record['raw'][$field])) ? explode('|^|',$record['raw'][$field]) : ((isset($attrs['default'])) ? $attrs['default'] : array());
	...
	public function GetCep($field, $selected = null, $parameters)
	{
	...
	public function GetFile($field, $selected = null, $parameter)
	{
	...
	public function GetSelect($field, $selected = null)
	{
		...
		$data['value'] = $this->DropdownLib->translate($this->fields_map[$field]['parameter'], $selected);
		$data['options'] = $this->DropdownLib->GetDropdownHTML($this->fields_map[$field]['parameter'], $selected);
	...
	public function GetMultiselect($field, $selected = array())
	{
		...
		$data['value'] = $this->DropdownLib->multitranslate($this->fields_map[$field]['parameter'], $selected);
		
		
		if(!$this->disabled_all){
			$data['options'] = $this->DropdownLib->GetMultiDropdownHTML($this->fields_map[$field]['parameter'], $selected);
		}else{
			$data['options'] = $this->DropdownLib->GetMultiDropdownHTMLSelected($this->fields_map[$field]['parameter'], $selected);
		}
